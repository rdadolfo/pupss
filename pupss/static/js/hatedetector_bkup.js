...
window.addEventListener('DOMContentLoaded', () => {
  setRunBtn(false); // Always start with button disabled
  ...
  if (saved) {
    try {
      ...
      // Restore filename label and results table
...
async function handleFile(file) {
  ...
  setRunBtn(true); // Enabled button after file loads
}
...
async function runDetection() {
  ...
  try {
    ...
    if (d.error) {
      ...
      // ✅ Save results + filename to sessionStorage 
      ...
      renderResults(d);
      // Check the flag in database for message status!
      if (d.is_cached) {
