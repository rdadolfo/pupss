// Global states for the table
let currentFilter = 'all';
let currentPage = 1;

// --- 1. On Page Load: Fetch Cards AND Initial Table ---
document.addEventListener("DOMContentLoaded", async () => {
    // A. Fetch the big summary numbers for the cards
    try {
    ...
    // B. Fetch the initial rows for the hidden table
    loadTableData();
})
...
function filterTable(filterType) {
    ...
    loadTableData(); // Fetch from server

    // 🌟 NEW: Automatically scroll down to the table!
    reportsTableContainer.scrollIntoView({ 
        behavior: 'smooth', // Makes it a smooth glide instead of a jarring jump
        block: 'start'      // Aligns the top of the table with the top of the screen
    });

}

// Fetches specific rows from Django
async function loadTableData() {
    const tbody = document.getElementById('tableBody');
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">⏳ Loading data...</td></tr>';

    try {
        // Ping our new API with the filter and page number
...
// --- 1. Update Next/Prev buttons ---
        ...
        // --- 2. Generate Numbered Buttons (Smart Window) ---
        const pageNumbersContainer = document.getElementById('pageNumbers');
        pageNumbersContainer.innerHTML = ''; // Clear old numbers

        let maxVisibleButtons = 5; // How many buttons to show at once
        ...
        for (let i = startPage; i <= endPage; i++) {
            ...
            const btnStyle = isActive
                ? `padding: 6px 12px; background: #007bff; color: white; border: 1px solid #007bff; border-radius: 4px; cursor: pointer; font-weight: bold;`
